...
function transformToStandardXML(xmlString, urlToName = {}) {
    ...
    // Find all {uri}attr= only within tag attributes
    ...
    for (let i = matches.length - 1; i >= 0; i--) {
        ...
        // then add xmlns (positions are updated after replacement)
        const beforeMatch = result.substring(0, clarkStart);
        const lastOpenTag = beforeMatch.lastIndexOf('<');
        const nextCloseTag = result.indexOf('>', clarkStart);
