...
function read(xmlString) {
    ...
    return xml
        ...
        .catch(function (err) {
          console.error('Could not parse xml string', err, xmlString);
          throw err;
        });
}
...
function simplifyClarkNotation(xmlString) {
    return transformToStandardXML(xmlString, xmlNamespaceMap);
}
function transformToStandardXML(xmlString, urlToName = {}) {
    ...
    // Find all {uri}attr= only within tag attributes (not in text content)
    ...
    for (let i = matches.length - 1; i >= 0; i--) {
        ...
        if (!uriToPrefix.has(uri)) {
            ...
            const nextCloseTag = result.indexOf('>', matchIndex);
        ...
        // Find the exact clark notation part within the match
        const clarkNotation = `{${uri}}${attrName}=`;
        const replacement = ` ${prefix}:${attrName}=`;

        // Calculate the position of clark notation within the full match
        const clarkStart = matchIndex + fullMatch.indexOf(clarkNotation);

        // Replace only the clark notation part
        result = result.substring(0, clarkStart) +
          replacement +
          result.substring(clarkStart + clarkNotation.length);
    }
